...
# Global Variables
state_names = ['FOOD_TYPE_FILLED', 'PRICE_FILLED', 'LOCATION_FILLED', 'FOOD_TYPE_CONF', 'PRICE_CONF',
'LOCATION_CONF', 'BEST_ACTION']

action_indices = {0:'REQUEST_FOOD_TYPE', 1:'REQUEST_PRICE',
2:'REQUEST_LOCATION', 3:'EXPLICIT_CONFIRM_FOOD_TYPE', 4:'EXPLICIT_CONFIRM_PRICE',
5:'EXPLICIT_CONFIRM_LOCATION'}

action_indices_invert = {'REQUEST_FOOD_TYPE':0, 'REQUEST_PRICE':1,
'REQUEST_LOCATION':2, 'EXPLICIT_CONFIRM_FOOD_TYPE':3, 'EXPLICIT_CONFIRM_PRICE':4,
'EXPLICIT_CONFIRM_LOCATION':5}

map_user_prob = {'REQUEST_FOOD_TYPE': ['PROVIDE_FOOD_TYPE', 'IRRELEVANT'],
                 'REQUEST_PRICE' : ['PROVIDE_PRICE','IRRELEVANT'], 'REQUEST_LOCATION': ['PROVIDE_LOCATION','IRRELEVANT'],
                 'EXPLICIT_CONFIRM_FOOD_TYPE': ['CONFIRM_POS_FOOD_TYPE','CONFIRM_NEG_FOOD_TYPE','IRRELEVANT'],
                 'EXPLICIT_CONFIRM_PRICE': ['CONFIRM_POS_PRICE','CONFIRM_NEG_PRICE','IRRELEVANT'], 'EXPLICIT_CONFIRM_LOCATION': ['CONFIRM_POS_LOCATION', 'CONFIRM_NEG_LOCATION', 'IRRELEVANT']}

mapping_user_to_state = {'PROVIDE_FOOD_TYPE': 0, 'PROVIDE_PRICE': 1, 'PROVIDE_LOCATION': 2, 'CONFIRM_POS_FOOD_TYPE': 3,'CONFIRM_POS_PRICE': 4, 'CONFIRM_POS_LOCATION': 5}

exploration_rate = 0.30
gamma = 0.99
alpha = 1

# exploration rate wrong changing, ## reward after each episode
...
def similated_user(episode_key, cur_state, action):
    ...
    if user_action == 'CONFIRM_POS_FOOD_TYPE' and cur_state[0] == 0:
        user_action = np.random.choice(['CONFIRM_NEG_FOOD_TYPE','IRRELEVANT'], p=[0.8, 0.2])

    elif user_action == 'CONFIRM_POS_PRICE' and cur_state[1] == 0:
        user_action = np.random.choice(['CONFIRM_NEG_PRICE','IRRELEVANT'], p=[0.8, 0.2])

    elif user_action == 'CONFIRM_POS_LOCATION' and cur_state[2] == 0:
        user_action = np.random.choice(['CONFIRM_NEG_LOCATION', 'IRRELEVANT'], p=[0.8, 0.2])
...
def update_state(cur_state, user_action):
    if user_action == 'IRRELEVANT':
        new_state = cur_state
    elif user_action == 'CONFIRM_NEG_PRICE':
        cur_state[1] = 0
        cur_state[4] = 0
        new_state = cur_state
    elif user_action == 'CONFIRM_NEG_LOCATION':
        cur_state[2] = 0
        cur_state[5] = 0
        new_state = cur_state
    elif user_action == 'CONFIRM_NEG_FOOD_TYPE':
        cur_state[0] = 0
        cur_state[3] = 0
        new_state = cur_state
    else:
...
def write_csv(filename):
    ...
    with open(filename, 'w') as csv_file:
        ...
        for i in keys_policy:
            writer.writerow({'FOOD_TYPE_FILLED': i[0], 'PRICE_FILLED': i[1], 'LOCATION_FILLED': i[2], 'FOOD_TYPE_CONF': i[3], 'PRICE_CONF': i[4],'LOCATION_CONF': i[5], 'BEST_ACTION' : policy_dictionary[i]})
...
def main():
    ...
    #print('\n', action_indices)
    #print(Q_values[tuple([1,1,0,1,1,0])], action_indices[np.argmax(Q_values[tuple([1,1,0,1,1,0])])], 'location')
    #print(Q_values[tuple([0,1,1,0,1,1])], action_indices[np.argmax(Q_values[tuple([0,1,1,0,1,1])])], 'type')
    #print(Q_values[tuple([1,0,1,1,0,1])], action_indices[np.argmax(Q_values[tuple([1,0,1,1,0,1])])], 'price')
    #print(Q_values[tuple([1,1,1,1,0,1])], action_indices[np.argmax(Q_values[tuple([1,1,1,1,0,1])])], 'ex_price')
